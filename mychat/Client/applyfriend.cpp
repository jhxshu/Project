...
{
    qDebug() << "=== ApplyFriend::SetSearchInfo START ===";
    qDebug() << "1. si is null? " << (si == nullptr);

    if (!si) {
        qDebug() << "ERROR: si is null";
        return;
    }

    qDebug() << "2. si->_name = " << si->_name;
    qDebug() << "3. ui pointer = " << ui;
    ...
    qDebug() << "4. ui->name_ed = " << ui->name_ed;
    qDebug() << "5. ui->back_ed = " << ui->back_ed;

    _si = si;

    qDebug() << "6. Getting UserMgr instance";
    auto userMgr = UserMgr::GetInstance();
    qDebug() << "7. UserMgr instance = ";

    auto applyname = userMgr->GetName();
    qDebug() << "8. applyname = " << applyname;

    auto bakname = si->_name;
    qDebug() << "9. bakname = " << bakname;

    qDebug() << "10. Setting text to name_ed";
    if (ui->name_ed) {
        ui->name_ed->setText(applyname);
        qDebug() << "11. name_ed setText done";
    } else {
        qDebug() << "ERROR: ui->name_ed is null!";
    }

    qDebug() << "12. Setting text to back_ed";
    if (ui->back_ed) {
        ui->back_ed->setText(bakname);
        qDebug() << "13. back_ed setText done";
    } else {
        qDebug() << "ERROR: ui->back_ed is null!";
    }

    qDebug() << "=== ApplyFriend::SetSearchInfo END ===";
}
